...
class CreateUpdateAnimalDataForm(forms.ModelForm):
    class Meta:
        model = Animal
        fields = (
            'animal_type', 'name', 'date', 'height', 'weight', 'passport',
        )
        widgets = {
            ...
            'passport': forms.TextInput(
                attrs={'pattern': '\\d*', 'maxlength': 7, 'minlength': 7}
            ),
...
class AnimalTypeFilterForm(forms.Form):
    animal_filter = forms.ModelChoiceField(
        TypeAnimal.objects.only('id', 'name'),
        empty_label='All',
        required=False,
    )
